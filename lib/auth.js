        ...
        let login = function() {
            request({
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    request({
                        url: FW.HOST+'/scripts/login_check.php',
                        method: 'POST',
                        form: {
                            fw_u_n: one,
                            fw_p_w: two
...
_.loadCookieFromDisk = function(username,password,callback) {
    ...
    fs.access(FW.APP_PREFS+'/token-'+hash+'.json', fs.F_OK, function(err){
        if(err) {
            client.error('Couldn\'t access token-'+hash+'.json in '+FW.APP_PREFS);
            callback('fail');
        }
        else {
            //If the file exists
            fs.readFile(FW.APP_PREFS+'/token-'+hash+'.json', 'utf8', function(err,data) {
                if (err) {
                    client.error('Couldn\'t read from token-'+hash+'.json in '+FW.APP_PREFS);
                    callback('fail');
                }
...
_.saveCookieToDisk = function(username,password,callback) {
    ...
    fs.writeFile(FW.APP_PREFS+'/token-'+hash+'.json', crypted, function(err) {
        if (err) {
            client.error('Couldn\'t write token-'+hash+'.json to '+FW.APP_PREFS);
            callback('fail');
        }

...
_.loadCookieFromDisk = function(username,password,callback) {
    ...
    fs.access('token-'+hash+'.json', fs.F_OK, function(err){
        if(err) {
            client.error('Couldn\'t access token-'+hash+'.json');
            callback('fail');
        }
        else {
            //If the file exists
            fs.readFile('token-'+hash+'.json', 'utf8', function(err,data) {
                if (err) {
                    client.error('Couldn\'t read from token-'+hash+'.json');
                    callback('fail');
                }
...
_.saveCookieToDisk = function(username,password,callback) {
    ...
    fs.writeFile('token-'+hash+'.json', crypted, function(err) {
        if (err) {
            client.error('Couldn\'t write token-'+hash+'.json to disk.');
            callback('fail');
        }
